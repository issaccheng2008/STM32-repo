...
#define STATE_FRAME_SIZE (sizeof(ProtocolHeader) + sizeof(RobotStatePayload) + 2U)
...
uint8_t JetsonUsbCdc_SendState(const RobotStatePayload *state)
{
    ...
    length = Protocol_EncodeState(state, state_frame, sizeof(state_frame));
    if (length == 0U) {
        return USBD_FAIL;
    }
    return CDC_Transmit_FS(state_frame, length);
}
